...
module.exports = grammar(C, {
  ...
  rules: {
    ...
    // Bare nullability-audit region markers that wrap most modern ObjC
    // headers. They are function-less macros (no arguments, no semicolon) and
    // must parse as a benign standalone node instead of being read as the type
    // of a declaration that then swallows the following `@interface`.
    nullability_region: _ => prec.dynamic(10, choice(
    ...
    declaration: $ => seq(
      ...
      repeat($._trailing_declaration_modifiers),
      ';',
    ),
    ...
    // Trailing declaration modifiers: everything a leading modifier may be,
    // plus availability-style attribute macros with a user-defined
    // (non-builtin) name. Placing `attribute_macro` only in the trailing
    // position (after the declarator, before `;`) avoids the pervasive
    // `IDENT(` ambiguity with type specifiers/declarators at the start of a
    // declaration while still catching project macros such as
    // `AF_API_AVAILABLE(ios(10))` or `MY_DEPRECATED("msg")`.
    _trailing_declaration_modifiers: $ => choice(
      $._declaration_modifiers,
      $.attribute_macro,
    ),

    attribute_macro: $ => prec.right(seq(
