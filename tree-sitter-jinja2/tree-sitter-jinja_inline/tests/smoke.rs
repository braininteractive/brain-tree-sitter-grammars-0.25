...
fn smoke_corpus_parses_cleanly() {
    ...
    let mut paths: Vec<_> = fs::read_dir(&corpus_dir)
        ...
        .filter(|path| path.extension().is_some_and(|ext| ext == "jinja_inline"))
        .collect();
    paths.sort();
    ...
    parser
        .set_language(&tree_sitter_jinja_inline::LANGUAGE.into())
...
fn expected_node_kinds_are_present() {
    ...
    let expected = [
        "line_statement", // # extends / # include / # set / # do
        "if_block",
        "for_block",
        "block_block",
        "macro_block",
    ...
    for entry in fs::read_dir(&corpus_dir).unwrap() {
        ...
        if path.extension().is_none_or(|ext| ext != "jinja_inline") {
            continue;
        }
        let source = fs::read_to_string(&path).unwrap();
