...
#[test]
fn query_constants_are_nonempty() {
    assert!(!tree_sitter_blade::HIGHLIGHTS_QUERY.trim().is_empty());
    assert!(!tree_sitter_blade::INJECTIONS_QUERY.trim().is_empty());
    assert!(!tree_sitter_blade::NODE_TYPES.trim().is_empty());
}
