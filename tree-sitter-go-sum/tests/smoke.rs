...
fn corpus_paths() -> Vec<PathBuf> {
    ...
    let mut paths: Vec<_> = fs::read_dir(&corpus_dir)
        ...
        .filter(|path| path.extension().is_some_and(|ext| ext == "sum"))
        .collect();
    paths.sort();
    paths
}

fn gosum_parser() -> Parser {
    let mut parser = Parser::new();
    parser
        .set_language(&tree_sitter_gosum::LANGUAGE.into())
...
fn expected_node_kinds_are_present() {
    ...
    for path in corpus_paths() {
        ...
        while let Some(node) = stack.pop() {
            if let Some(idx) = expected_kinds.iter().position(|kind| *kind == node.kind()) {
                seen[idx] = true;
            }
            let mut cursor = node.walk();
