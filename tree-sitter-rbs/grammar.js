...
module.exports = grammar({
  ...
  rules: {
    ...
    method_type_parameters: $ => seq("[", commaSep1($.method_type_parameter), "]"),

    // Method-level type parameters accept an upper bound (`T < Numeric`),
    // mirroring the class-level `module_type_parameter` rule. Per the RBS
    // grammar, variance (`out`/`in`) and `unchecked` are only valid on
    // declaration-level type parameters, so they are intentionally omitted here.
    method_type_parameter: $ => seq(
      $.type_variable,
      optional($.generics_bound),
    ),
    ...
    // Bang (destructive) method names, e.g. `sort!`, `map!`. Mirrors the
    // existing `?`-predicate suffix rule; these are pervasive in Ruby core.
    identifier_bang: $ => seq($.identifier, token.immediate('!')),
    ...
    method_name: $ => choice(
      ...
      $.constant_setter,
      $.operator,
      // Backtick-quoted method name (single line). Excluding newlines stops
      // this token from greedily consuming across lines when it is really the
      // bare backtick operator method, e.g. `def self?.`: (String) -> String`.
      /`[^`\n]+`/,
    ),
