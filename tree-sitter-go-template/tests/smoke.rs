...
fn smoke_corpus_parses_cleanly() {
    ...
    let mut paths: Vec<_> = fs::read_dir(&corpus_dir)
        ...
        .filter(|path| path.extension().is_some_and(|ext| ext == "tmpl"))
        .collect();
    paths.sort();
    ...
    parser
        .set_language(&tree_sitter_go_template::LANGUAGE.into())
...
fn expected_top_level_node_kinds_are_present() {
    ...
    for entry in fs::read_dir(&corpus_dir).unwrap() {
        ...
        if path.extension().is_none_or(|ext| ext != "tmpl") {
            continue;
        }
        let source = fs::read_to_string(&path).unwrap();
        ...
        while let Some(node) = stack.pop() {
            if let Some(pos) = expected_kinds.iter().position(|k| *k == node.kind()) {
                seen[pos] = true;
            }
            let mut cursor = node.walk();
