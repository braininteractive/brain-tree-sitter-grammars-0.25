...
fn smoke_corpus_parses_cleanly() {
    ...
    let mut paths: Vec<_> = fs::read_dir(&corpus_dir)
        ...
        .filter(|path| path.extension().is_some_and(|ext| ext == "liquid"))
        .collect();
    paths.sort();
    ...
    parser
        .set_language(&tree_sitter_liquid::LANGUAGE.into())
        .expect("Error loading liquid parser");
...
fn expected_top_level_node_kinds_are_present() {
    ...
    for entry in fs::read_dir(&corpus_dir).unwrap() {
        ...
        while let Some(node) = stack.pop() {
            if expected_kinds.contains(&node.kind()) {
