...
fn smoke_corpus_parses_cleanly() {
    ...
    parser
        .set_language(&tree_sitter_move_on_aptos::LANGUAGE.into())
        .expect("Error loading MoveOnAptos parser");
...
fn expected_top_level_node_kinds_are_present() {
    ...
    let mut seen: std::collections::HashMap<&str, bool> =
        expected_kinds.iter().map(|k| (*k, false)).collect();

    for entry in fs::read_dir(&corpus_dir).unwrap() {
        let path = entry.unwrap().path();
        if path.extension().is_none_or(|ext| ext != "move") {
            continue;
        }
        let source = fs::read_to_string(&path).unwrap();
        ...
        while let Some(node) = stack.pop() {
            if let Some(flag) = seen.get_mut(node.kind()) {
                *flag = true;
            }
            let mut cursor = node.walk();
