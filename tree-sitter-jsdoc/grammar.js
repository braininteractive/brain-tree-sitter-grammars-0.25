...
module.exports = grammar({
  ...
  externals: $ => [
    $.type,
    $.code_block_line,
    // Opaque, free-text body for `@example` tags. Scanned by the external
    // scanner so arbitrary runnable code (semicolons, `//` comments, `=>`,
    // multiple lines) is tolerated without being parsed as structured content.
    $.example_body,
  ],
  ...
  rules: {
    ...
    tag: $ => choice(
      // `@example`: its body is an opaque, free-text region tolerating arbitrary
      // runnable code. A leading fenced code block (```) is still parsed as a
      // structured `description`/`code_block`; the external scanner defers in
      // that case (see `scan_example_body` in scanner.c).
      seq(
        alias($.tag_name_example, $.tag_name),
        optional(choice($.description, $.example_body)),
      ),
    ...
    // Matched as an explicit string token so it takes lexer precedence over the
    // generic `tag_name` regex, routing `@example` into the opaque-body branch.
    tag_name_example: _ => '@example',
