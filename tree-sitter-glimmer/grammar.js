export default grammar({
  ...
  rules: {
    ...
    // The standard HTML void elements. Per the HTML spec these elements never
    // have a closing tag and may appear WITHOUT a self-closing slash
    // (e.g. `<br>`, `<input type="text">`, `<img src="x">`). This is idiomatic
    // and extremely common in real Ember/Glimmer templates.
    //
    // Modeled as a choice of plain string literals (not wrapped in `token()`),
    // so that for one of these exact names the lexer prefers it over the
    // generic `tag_name` regex: on an equal-length match a string literal wins
    // over a regexp. Any longer name (e.g. `input-group`, `hr-thing`,
    // `img.foo`) still lexes as a normal `tag_name`, because tree-sitter
    // prefers the longer match before that tie-break is ever consulted.
    _void_element_name: () =>
      choice(
        "area",
        "base",
    ...
    // "Void" elements have no closing tag. Two forms are accepted:
    //   1. Any element may be self-closed:            `<div />`, `<Foo />`, `<img />`
    //   2. A standard HTML void element may be closed with a bare `>` (or a
    //      self-closing slash):                        `<br>`, `<input type="text">`
    element_node_void: ($) =>
      choice(
        seq(
          "<",
        ...
        seq(
          "<",
          alias($._void_element_name, $.tag_name),
          repeat(
            choice(
              $.attribute_node,
              $.mustache_statement,
