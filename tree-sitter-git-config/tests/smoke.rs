...
fn smoke_corpus_parses_cleanly() {
    ...
    let mut paths: Vec<_> = fs::read_dir(&corpus_dir)
        ...
        .filter(|path| path.extension().is_some_and(|ext| ext == "gitconfig"))
        .collect();
    paths.sort();
    ...
    parser
        .set_language(&tree_sitter_git_config::LANGUAGE.into())
        .expect("Error loading git-config parser");
...
fn expected_top_level_node_kinds_are_present() {
    ...
    for entry in fs::read_dir(&corpus_dir).unwrap() {
        ...
        if path.extension().is_none_or(|ext| ext != "gitconfig") {
            continue;
        }
        let source = fs::read_to_string(&path).unwrap();
