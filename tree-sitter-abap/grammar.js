  ...
  rules: {
    ...
    form_definition: $ =>
      seq(
        kw("form"),
        field("name", $.name),
        field("tables_parameters", optional($._form_tables)),
        field("using_parameters", optional($._form_using)),
        field("changing_parameters", optional($._form_changing)),
        ".",
        repeat($._implementation_statement),
        kw("endform"),
        "."
    ...
    _form_tables: $ => seq(kw("tables"), repeat1($.form_parameter)),

    _form_using: $ => seq(kw("using"), repeat1($.form_parameter)),

    _form_changing: $ => seq(kw("changing"), repeat1($.form_parameter)),

    form_parameter: $ =>
      seq(
        choice(seq(kw("value"), "(", $.name, ")"), $._operand),
        optional(
          choice($._typing, seq(kw("structure"), alias($.name, $.type)))
        )
      ),
    ...
    perform_statement: $ =>
      seq(
        ...
        optional(
          choice(
            seq(token.immediate("("), field("program", $.name), ")"),
            seq(kw("in"), kw("program"), field("program", $.name))
          )
        ),
        optional(seq(kw("if"), kw("found"))),
        field("tables_parameters", optional($._perform_tables)),
        field("using_parameters", optional($._perform_using)),
        field("changing_parameters", optional($._perform_changing)),
        "."
      ),

    _perform_tables: $ => seq(kw("tables"), repeat1($._data_object)),

    _perform_using: $ => seq(kw("using"), repeat1($._perform_argument)),

    _perform_changing: $ => seq(kw("changing"), repeat1($._perform_argument)),

    _perform_argument: $ =>
      choice($.numeric_literal, $.character_literal, $._data_object),
