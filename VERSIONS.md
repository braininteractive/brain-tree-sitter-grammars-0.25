...
## Section K — docs & infra (added 2026-08-01, tag `v0.25.0-fork.11`)

| Grammar (dir) | Crate name | Upstream @ rev | ABI | Corpus |
| --- | --- | --- | --- | --- |
| tree-sitter-latex | `tree-sitter-latex` | latex-lsp @ `7e0ecdc` | 15 (parser committed fork-side) + LanguageFn | 104/104 |
| tree-sitter-bibtex | `tree-sitter-bibtex` | latex-lsp @ `8d04ed2` | 15 (pin) | smoke-only (no upstream corpus) |
...
Rejected after evaluation: **mdx** (stale md fork; same crate name & C symbols as tree-sitter-md — guaranteed link collision), **systemd** (no viable grammar; units are INI-family — use `tree-sitter-ini`), **batch/velocity/stylus/zsh** (no grammars exist).
...
| --- | --- | --- | --- | --- | --- |
| hcl | v0.25.0-fork.1 | 15 | MichaHoffmann/tree-sitter-hcl @ `64ad627` (2026-01-17) | pending (symbol rename is fork-side) | yes. **fork.6: C symbols renamed to `tree_sitter_hcl_ng`** + `links` key — ends the latent link collision with crates.io tree-sitter-hcl (ast-grep). Rust API unchanged, node kinds unchanged, 102/102 |
