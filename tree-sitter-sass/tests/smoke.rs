...
fn corpus_paths() -> Vec<PathBuf> {
    ...
    let mut paths: Vec<_> = fs::read_dir(&dir)
        ...
        .map(|e| e.unwrap().path())
        .filter(|p| p.extension().is_some_and(|x| x == "sass"))
        .collect();
    paths.sort();
    paths
}
...
fn make_parser() -> Parser {
    ...
    parser
        .set_language(&tree_sitter_sass::LANGUAGE.into())
...
fn expected_node_kinds_are_present() {
    let expected = ["at_rule", "declaration", "mixin_statement", "include_statement"];
    ...
    // Require at least the declaration node (universal); others are best-effort
    // across the corpus depending on the grammar's exact kind names.
    assert!(
        seen.contains("declaration"),
        "no `declaration` node found anywhere in the corpus; saw {:?}",
        seen
    );
}
