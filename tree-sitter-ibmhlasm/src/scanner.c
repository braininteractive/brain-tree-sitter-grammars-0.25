...
{                
  ...
  while (keep_parsing_func_ptr(lexer, continuation_line, in_quoted_string)) {
    ...
    if (iswcntrl(lexer->lookahead) && continuation_line == true) { // TODO: Maybe this should be a while?
      ...
      while (continuation_line && lexer->eof(lexer) == 0) {
        ...
        // Read in the symbol on the continuation line. The EOF guard is
        // essential: at end of input lexer->lookahead is 0 and iswspace(0)
        // is false, so without it this loop spins forever (advance() cannot
        // move past EOF) whenever a continuation is expected but the file
        // ends first — e.g. a line with a non-blank in column 72 at EOF.
        while (lexer->eof(lexer) == 0 && iswspace(lexer->lookahead) == 0) {
...
bool tree_sitter_ibmhlasm_external_scanner_scan(void *payload, TSLexer *lexer,
  ...
  if (valid_symbols[COMMENT] || valid_symbols[NAME]) {
    if (parse_w_cont(lexer, COMMENT) == true) {
      ...
      // Leave '&VAR' (variable symbols) and '.SEQ' / '.*' (sequence symbols,
      // macro comments) to the internal lexer so they get their own nodes
