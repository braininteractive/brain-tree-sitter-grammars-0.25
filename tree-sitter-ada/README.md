...
> **Provenance (fork):** forked from <https://github.com/briot/tree-sitter-ada>
> (commit `6b58259`, 2025-12-16).
> **Changes vs upstream:** `parser.c` regenerated with tree-sitter-cli 0.25.10
> (ABI 15); Cargo package version set to `0.25.0`; Rust doc-example filled in
> with a tiny valid snippet; added a synthetic smoke-test corpus
> (`test/corpus_smoke/`) and `tests/smoke.rs`. No grammar-rule changes — a
> completeness assessment (15 synthetic samples covering package specs/bodies,
> subprograms, records/arrays/access types, generics, tasks/protected objects,
> exceptions, attributes, aspect clauses, tagged types, and Ada 2022
> conditional/quantified/declare expressions) found no common-Ada holes.
> **Rebase policy:** binding-only diff, rebased onto upstream when it moves;
> retired if upstream ships an equivalent ABI-15 regeneration.
