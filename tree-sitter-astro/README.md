...
> legacy `pub fn language()` shape to the `tree-sitter-language` `LanguageFn` API
> (`pub const LANGUAGE`); `Cargo.toml`: `tree-sitter ~0.20.10` hard dependency replaced by
> `tree-sitter-language = "0.1"` (with `tree-sitter = "0.25"` as dev-dependency only),
> crate version aligned to `0.25.0`, repository URL corrected to the actual upstream;
> upstream corpus moved `corpus/` -> `test/corpus/` (CLI 0.25 layout); added a synthetic
> smoke-test corpus (`test/corpus_smoke/`, 12 files) and `tests/smoke.rs`. External C
> scanner kept verbatim. No grammar-rule changes. Package name `tree-sitter-astro` kept.
> Upstream corpus: 22/22; smoke corpus: 0.00% error nodes.
> **Rebase policy:** binding/regen/tests diff, rebased onto upstream when it moves; retired
> if upstream ships an equivalent 0.25 binding.
