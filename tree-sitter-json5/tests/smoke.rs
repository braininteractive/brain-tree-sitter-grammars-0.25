...
fn expected_top_level_node_kinds_are_present() {
    ...
    for entry in fs::read_dir(&corpus_dir).unwrap() {
        ...
        if path.extension().is_none_or(|ext| ext != "json5") {
            continue;
        }
        let source = fs::read_to_string(&path).unwrap();
        ...
        while let Some(node) = stack.pop() {
            match node.kind() {
                "object" => seen_object = true,
                "array" => seen_array = true,
                "member" => seen_member = true,
                "comment" => seen_comment = true,
                _ => {}
            }
            let mut cursor = node.walk();
    ...
    assert!(seen_object, "no `object` node found anywhere in the corpus");
    assert!(seen_array, "no `array` node found anywhere in the corpus");
    assert!(seen_member, "no `member` node found anywhere in the corpus");
    assert!(seen_comment, "no `comment` node found anywhere in the corpus");
}
